...
// Response Messages
const MESSAGES = {
  unauthorizedError: "You are not authorized to mute users",
  successMessage: (username) => `${username} is now muted`,
};
 ...
 * @param {Object} targetUser - The user to be muted.
 */
const muteUser = async (ws, channelId, user, targetUser) => {
  ...
  if (channelRole && (channelRole.role === "HOST" || channelRole.role === "SUPERADMIN")) {
    await db.ChannelRole.update({ role: "MUTED" }, { where: { profileId: targetUser.id, channelId: channelId } });
